...
async function runDag() {
  ...
  while (state.status === 'running') {
    ...
    if (state.current_phase === 'execution-loop') {
      ...
      let testExit = await adapter.run('test-engineer', path.join(require('os').homedir(), '.agent-system', 'roles', `test-engineer.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast);
      ...
      let implExit = await adapter.run('implementation', path.join(require('os').homedir(), '.agent-system', 'roles', `implementation.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast);
      if (implExit !== 0) {
      ...
      let revExit = await adapter.run('reviewer', path.join(require('os').homedir(), '.agent-system', 'roles', `reviewer.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast);
      if (revExit === 0) {
      ...
      let exitCode = await adapter.run(state.current_phase, systemPromptPath, runContext, env, mcpConfig, modelTarget);
