...
async function runDag() {
  console.log("[Orchestrator] Booting state machine...");
  ...
  if (state.status === 'completed') {
    console.log("[Orchestrator] Pipeline is already marked as completed. Reset state or modify tasks.json to continue.");
    process.exit(0);
  }
  ...
  while (state.status === 'running') {
    ...
    const isDebug = env.SF_DEBUG === "true";
    ...
    if (state.current_phase === 'execution-loop') {
      ...
      let testExit = await adapter.run('test-engineer', path.join(require('os').homedir(), '.agent-system', 'roles', `test-engineer.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast, isDebug);
      ...
      let implExit = await adapter.run('implementation', path.join(require('os').homedir(), '.agent-system', 'roles', `implementation.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast, isDebug);
      if (implExit !== 0) {
      ...
      let revExit = await adapter.run('reviewer', path.join(require('os').homedir(), '.agent-system', 'roles', `reviewer.md`), activeTask.context_slice, env, mcpConfig, pipeline.model_tiering.fast, isDebug);
      if (revExit === 0) {
      ...
      let exitCode = await adapter.run(state.current_phase, systemPromptPath, runContext, env, mcpConfig, modelTarget, isDebug);
